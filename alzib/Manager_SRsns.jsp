...
<script>
    function search() {
      ...
      window.location.href = "Manager_SRsns.jsp?keyword=" + encodeURIComponent(keyword);
    }
...
<%
    ...
    try {
        ...
        // Category 값이 있을 경우에만 WHERE 절에 추가
        if (Category != null && !Category.isEmpty()) {
            sql += " WHERE Category = ?";
        }

        // 키워드에 따라 검색 필드를 선택하고 LIKE 연산자를 사용하여 검색
        sql += " AND " + field + " LIKE ? ORDER BY Sdate DESC";

        pstmt = conn.prepareStatement(sql);

        // Category 값이 있을 경우에만 바인딩
        if (Category != null && !Category.isEmpty()) {
...
<div class="wrap">
    ...
    <% if (Category != null && !Category.isEmpty()) { %>
        <% if (Category.equals("free")) { %>
            자유게시판 "<font color="#0047A9"><b> <%=keyword%> </b></font>" 검색 결과
        <% } else if (Category.equals("information")) { %>
            정보게시판 "<font color="#0047A9"><b> <%=keyword%> </b></font>" 검색 결과
        <% } else if (Category.equals("complain")) { %>
            하소연게시판 "<font color="#0047A9"><b> <%=keyword%> </b></font>" 검색 결과
        <% } else if (Category.equals("q")) { %>
            질문게시판 "<font color="#0047A9"><b> <%=keyword%> </b></font>" 검색 결과
        <% } else { %>
            후기게시판 "<font color="#0047A9"><b> <%=keyword%> </b></font>" 검색 결과
        <% } %>
    <% } else { %>
        "<b><%=keyword%></b>" 검색 결과
    <% } %>
