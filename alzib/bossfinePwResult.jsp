...
<%
    ...
    try {
        ...
        String sql = "SELECT bPasswd FROM boss WHERE bId = ? AND bName = ? AND bBusiness = ? AND bEmail = ?";
        pstmt = con.prepareStatement(sql);
        pstmt.setString(1, bId);
        pstmt.setString(2, bName);
        pstmt.setString(3, bBusiness);
        pstmt.setString(4, bEmail);

        rs = pstmt.executeQuery();

        if (rs.next()) {
            bPassword = rs.getString("bPasswd");
        }
