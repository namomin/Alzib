...
try {
        ...
        list_index = currentPage - 1; // 현재 페이지 수, 0부터 시작하도록 조정

        int pageSize = 5; // 페이지당 보여줄 게시글 수

        String jsqlCategory = "SELECT * FROM sns ORDER BY Sdate DESC LIMIT ?, ?";

        PreparedStatement pstmt = con.prepareStatement(jsqlCategory);
        pstmt.setInt(1, list_index * pageSize); // 시작 인덱스 계산
        pstmt.setInt(2, pageSize); // 페이지당 보여줄 게시글 수
        ...
        <div class="popular">
            <%
                ...
                while (popularRs.next()) {
                    ...
                    String Category = popularRs.getString("Category");
    ...
    <div class="contents">
        <div class="left_wrap">
            <div class="category">
                <table>
                    <tr>
                        <td id="present_category"><a href="snsmain.jsp"><font color="white">모든글</font></a></td>
...
<div class="page">
        <%-- 처음 페이지로 이동 --%>
        <% if (currentPage > 1) { %>
            <a href="snsmain.jsp?currentPage=1">
                < &nbsp;
            </a>
        <% } %>
        ...
        %>
            <a href="snsmain.jsp?currentPage=<%= j %>"><%= j %></a> &nbsp;
        <%
                }
            }
        %>
        ...
        <% if (currentPage < totalPage) { %>
            <a href="snsmain.jsp?currentPage=<%= totalPage %>">
                >
            </a>
        <% } %>
    </div>
