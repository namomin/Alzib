...
%>
<div class="nav">
		<span><a href="main.jsp">홈</a> > 편의기능 > <a href="Calendar_alba.jsp">근무 시간표</a></span>
</div>
<h2 align="center" style="margin-bottom: 40px; margin-top: 0;">근무시간표</h2>
...
<div id="salaryInfo1" style="border-bottom: 1px solid #ccc; padding: 10px; ">
    ...
    <p>
        6월 <%= CName %>에서 총 <%= totalHours %>시간 근무<br>
        전체 월급: <%= Paymm1 %>원
    </p>
    <p>근무내역:</p>
    <ul>
		...
		while (calendarRs2.next()) { 
			...
			%>
            <li><%= calendarRs2.getString("CCalendar") %>에 <%= CName %>에서 총 <%= calendarRs2.getInt("CTime") %>시간 근무 급여: <%= Paymm2 %>원</li>
        <% } %>
    </ul>
	</div>
</div>
...
<div id="salaryInfo1" style="border-bottom: 1px solid #ccc; padding: 10px;">
    ...
    <h3><%= CName %> 근무지 <%= CSalary %> 내역</h3>
    <p>
        6월3일~6월9일에 <%= CName %>에서 총 <%= totalHours1 %>시간 근무
        주급: <%= Payww1 %>원<br>
    </p>
...
%>
    ...
    <p>
        6월10일~6월16일에 <%= CName %>에서 총 <%= totalHours2 %>시간 근무
        주급: <%= Payww2 %>원<br>
    </p>
...
%>
    ...
    <p>
        6월17일~6월23일에 <%= CName %>에서 총 <%= totalHours3 %>시간 근무
        주급: <%= Payww3 %>원<br>
    </p>
...
%>
    ...
    <p>
        6월24일~6월30일에 <%= CName %>에서 총 <%= totalHours4 %>시간 근무
        주급: <%= Payww4 %>원<br>
    </p>
...
%>
  ...
  <%--  <p> --%>
  <%--      6월0일~6월0일에 <%= CName %>에서 총 <%= totalHours6 %>시간 근무 --%>
  <%--      주급: <%= Payww6 %>원<br> --%>
  <%--  </p> --%>
...
%>
    ...
    <ul>
		...
		%>
            <li><%= calendarRs5.getString("CCalendar") %>에 <%= CName %>에서 총 <%= calendarRs5.getInt("CTime") %>시간 근무급여: <%= Payww5 %>원</li>
        <% } %>
    </ul>
	</div>
</div>
...
<div id="salaryInfo1" style="border-bottom: 1px solid #ccc; padding: 10px;">
    ...
    <ul>
		...
		%>
            <li><%= calendarRs.getString("CCalendar") %>에 <%= CName %>에서 총 <%= calendarRs.getInt("CTime") %>시간 근무 일급: <%= Paydd %>원</li>
        <% } %>
    </ul>
	</div>
</div>
