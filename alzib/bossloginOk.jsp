...
<%
    ...
    try {
        ...
        // SQL 쿼리 실행
        String jsql = "SELECT * FROM boss WHERE bId = ? AND bPasswd = ?";
...
<%
    if (loginSuccess) { 
		response.sendRedirect("index_boss.jsp");
%>
<%
    } else {
%>
    <p>아이디 또는 비밀번호가 틀렸습니다.</p>
    <button onclick="location.href='bosslogin.html'">로그인 화면으로 돌아가기</button>
<%
    }
%>
</body>
