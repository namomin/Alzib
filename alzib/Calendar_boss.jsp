...
<script>
	window.addEventListener('DOMContentLoaded', (event) => {
        // 페이지가 로드되면 showSalary 함수를 호출합니다.
        showSalary();
    });
</script>
</head>
...
<%
    ...
    try {
        ...
        String query = "SELECT CTime, CSchedule, CCalendar, working FROM Calendar WHERE YEAR(CCalendar) = ? AND MONTH(CCalendar) = ? AND CId = ?";
...
%>
<div class="nav">
		<span><a href="main.jsp">홈</a> > 편의기능 > <a href="Calendar_boss.jsp">근무자 시간표</a></span>
</div>

<h2 align="center" style="margin-bottom: 20px; margin-top: 0;">근무자 시간표</h2>
...
<table class="calendar">
    <thead>
        <tr>
            <th colspan="7">
                ...
                <%= new java.text.SimpleDateFormat("yyyy년 MMMM").format(cal.getTime()) %>
    ...
    <tbody>
		...
		int dayCountNextMonth = 0; // 다음 달의 첫째 날부터 시작하도록 변수를 추가합니다.
        int dayCount = 1;
        for (int i = 0; i < 6; i++) {
    out.print("<tr>");
    for (int j = 1; j <= 7; j++) {
        if ((i == 0 && j < firstDayOfWeek) || dayCount > lastDayOfMonth) {
    int day = (j <= firstDayOfWeek) ? (lastDayOfPrevMonth - firstDayOfWeek + j + 1) : (dayCountNextMonth);
    dayCountNextMonth++; // 다음 달로 넘어갔을 때 1을 증가시킵니다.
    out.print("<td style='color: white;'>" + day + "</td>");
            ...
            if (scheduleMap.containsKey(date)) {
                ...
                for (String[] event : events) {
                    ...
                    // 배경색을 적용한 스케줄을 출력합니다.
                    out.print("<span style='background-color: " + backgroundColor + ";'>" + placeName + " " + event[0] + "</span><br>");
                }
            }
...
<center>
    <br><br>
	<button onclick="showPlace()">근무자 추가/삭제</button>
	<button onclick="showBulkEventForm()">일괄 일정 추가</button>
...
<div id="eventForm" style="display: none;" class="form">
	<div id="placeForm1">
    <h2>근무자 일정 추가</h2>
    <form action="CalendarResult_boss.jsp" method="post">

		<label for="cTimeb">근무 시간:</label>
		<select name="cTime1">
	...
	<div id="placeForm2">
	 <h2>근무자 일정 삭제</h2>
        <form action="CalendarDelete_boss.jsp" method="post">
        ...
        %>
			...
			<div><font color="gray">선택한 날짜의 근무자 일정을 삭제할 수 있습니다.</font></div>
    ...
    <div id="bulkForm" style="display: none;" class="form">
        <h3>일괄 일정 추가</h3>
		<form action="CalendarsResult1_boss.jsp" method="post">
		<label for="cTimeab">근무 시간:</label>
		<select name="cTimea1">
            <option value="1">1시</option>
            <option value="2">2시</option>
            <option value="3">3시</option>
        ...
        <label for="bulkCSchedule">근무자:</label>
	...
	<div id="placeForm1" style="height: 350px;">
        <h2>근무자 추가</h2>
        <form action="CalplaceResult_boss.jsp" method="post">
            <label for="cname">근무자 이름:</label>
            ...
            <select id="ccolor" name="ccolor">
                ...
                <option value="Yellowgreen">초록색</option>
		...
		<div id="placeForm2">
        <h2>근무자 삭제</h2>
        <form action="CalplaceDelete_boss.jsp" method="post">
            <label for="deleteP">근무자 선택:</label>
        ...
        %>
			...
			<div><font color="red">※근무자를 삭제하면 해당 근무지 일정도 삭제 됩니다※</font></div>
            <input type="submit" value="삭제">
        </form>
		</div>
    </div>
...
<div id="salaryInfo1" style="border-bottom: 1px solid #ccc; padding: 10px; ">
	<div id="salaryInfo2">
    <h3><%= CName %> 근무자 <%= CSalary %> 내역</h3>
    <p>
        6월 총 <%= totalHours %>시간 근무<br>
        전체 월급: <%= Paymm1 %>원
    </p>
    <p>근무내역:</p>
    <ul>
		...
		while (calendarRs2.next()) { 
			...
			%>
            <li><%= calendarRs2.getString("CCalendar") %>에 총 <%= calendarRs2.getInt("CTime") %>시간 근무 급여: <%= Paymm2 %>원</li>
        <% } %>
    </ul>
	</div>
</div>
...
<div id="salaryInfo1" style="border-bottom: 1px solid #ccc; padding: 10px;">
    ...
    <h3><%= CName %> 근무자 <%= CSalary %> 내역</h3>
    <p>
        6월3일~6월9일에 총 <%= totalHours1 %>시간 근무
        주급: <%= Payww1 %>원<br>
    </p>
...
%>
    ...
    <p>
        6월10일~6월16일에 총 <%= totalHours2 %>시간 근무
        주급: <%= Payww2 %>원<br>
    </p>
...
%>
    ...
    <p>
        6월17일~6월23일에 총 <%= totalHours3 %>시간 근무
        주급: <%= Payww3 %>원<br>
    </p>
...
%>
    ...
    <p>
        6월24일~6월30일에 총 <%= totalHours4 %>시간 근무
        주급: <%= Payww4 %>원<br>
    </p>
...
%>
    ...
    <%--<p>--%>
    <%--    5월27일~6월2일에 <%= CName %>에서 총 <%= totalHours6 %>시간 근무--%>
    <%--    주급: <%= Payww6 %>원<br>--%>
    <%--</p>--%>
...
%>
    ...
    <ul>
		...
		%>
            <li><%= calendarRs5.getString("CCalendar") %>에 총 <%= calendarRs5.getInt("CTime") %>시간 근무급여: <%= Payww5 %>원</li>
        <% } %>
    </ul>
	</div>
</div>
...
<div id="salaryInfo1" style="border-bottom: 1px solid #ccc; padding: 10px;">
    ...
    <ul>
		...
		%>
            <li><%= calendarRs.getString("CCalendar") %>에 총 <%= calendarRs.getInt("CTime") %>시간 근무 일급: <%= Paydd %>원</li>
        <% } %>
    </ul>
	</div>
</div>
...
<script>
    ...
    function selectDate(dateElement, date) {
        ...
        selectedDate = date;
        selectedDateElement.style.backgroundColor = '#eeeeee'; // 선택된 날짜를 노란색으로 표시
    }
